describe "model: BinderyIdentity", ->
  ...
  describe "query", ->
    it "should query identities in DataBindery", ->
      ...
      @$httpBackend.expectGET('http://bindery.cocupu.com/identities.json?q=simone').respond(sampleResponse)
    ...
    it "should allow querying for current user's identities", ->
      ...
      @$httpBackend.expectGET('http://bindery.cocupu.com/identities.json?q=current_user').respond(sampleResponse)
